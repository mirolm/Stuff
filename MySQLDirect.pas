...
type
  ...
  TMySqlSelectDb = function(mysql: PMYSQL; const db: PChar): Integer; stdcall;
  TMySqlClose = procedure(mysql: PMYSQL); stdcall;
  TMySqlPing = function(mysql: PMYSQL): Integer; stdcall;
  TMySqlRealQuery = function(mysql: PMYSQL; const stmt_str: PChar; length: LongWord): Integer; stdcall;
  TMySqlAffectedRows = function(mysql: PMYSQL): Int64; stdcall;
  TMySqlInsertId = function(mysql: PMYSQL): Int64; stdcall;
  TMySqlEscapeString = function(_to: pChar; const from: pChar; length: LongWord): LongWord; stdcall;
...
begin
  // Function mysql_info Can Be Added If
...
begin
  ...
  begin
    ...
    if (AnsiCompareText(FieldName, FieldDest) = 0) then
    begin
      Result := i;
...
begin
  ...
  try
    if LoadLib(LibMySqlHandle, LIB_MYSQL) then
    begin
      if not LoadFunc(LibMySqlHandle, @mysql_errno, FUN_MYSQL_ERR_NO) then
      begin
        Exit;
      end;

      if not LoadFunc(LibMySqlHandle, @mysql_error, FUN_MYSQL_ERROR) then
      begin
        Exit;
      end;

      if not LoadFunc(LibMySqlHandle, @mysql_sqlstate, FUN_MYSQL_SQL_STATE) then
      begin
        Exit;
      end;

      if not LoadFunc(LibMySqlHandle, @mysql_init, FUN_MYSQL_INIT) then
      begin
        Exit;
      end;
      ...
      if not LoadFunc(LibMySqlHandle, @mysql_select_db, FUN_MYSQL_SELECT_DB) then
      begin
        Exit;
      end;

      if not LoadFunc(LibMySqlHandle, @mysql_close, FUN_MYSQL_CLOSE) then
      begin
        Exit;
      end;

      if not LoadFunc(LibMySqlHandle, @mysql_ping, FUN_MYSQL_PING) then
      begin
        Exit;
      end;

      if not LoadFunc(LibMySqlHandle, @mysql_real_query, FUN_MYSQL_REAL_QUERY) then
      begin
        Exit;
      end;

      if not LoadFunc(LibMySqlHandle, @mysql_affected_rows, FUN_MYSQL_AFFECTED_ROWS) then
      begin
        Exit;
      end;

      if not LoadFunc(LibMySqlHandle, @mysql_insert_id, FUN_MYSQL_INSERT_ID) then
      begin
        Exit;
      end;

      if not LoadFunc(LibMySqlHandle, @mysql_escape_string, FUN_MYSQL_ESCAPE_STRING) then
      begin
        Exit;
      end;

      if not LoadFunc(LibMySqlHandle, @mysql_store_result, FUN_MYSQL_STORE_RESULT) then
      begin
        Exit;
      end;

      if not LoadFunc(LibMySqlHandle, @mysql_free_result, FUN_MYSQL_FREE_RESULT) then
      begin
        Exit;
      end;

      if not LoadFunc(LibMySqlHandle, @mysql_fetch_row, FUN_MYSQL_FETCH_ROW) then
      begin
        Exit;
      end;

      if not LoadFunc(LibMySqlHandle, @mysql_fetch_lengths, FUN_MYSQL_FETCH_LENGTHS) then
      begin
        Exit;
      end;

      if not LoadFunc(LibMySqlHandle, @mysql_num_fields, FUN_MYSQL_NUM_FIELDS) then
      begin
        Exit;
      end;

      if not LoadFunc(LibMySqlHandle, @mysql_fetch_field_direct, FUN_MYSQL_FETCH_FIELD_DIRECT) then
      begin
        Exit;
      end;
    end;
